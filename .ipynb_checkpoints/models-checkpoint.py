...
class FrozenResNetAndFinetune(nn.Module):
    def __init__(self, bb_model, train_bb=False, finetune_shape=32, num_classes=101, verbose=False):
        super().__init__()
        self.train_bb = train_bb
        self.bb_model = bb_model
        self.bb_model.fc = nn.Identity()
        ...
        mul = 2
        my_blocks_output_shape = {"l1": finetune_shape,
                                  "l2": int(finetune_shape * mul),
                                  "l3": int(finetune_shape * 2 * mul),
                                  "l4": int(finetune_shape * 4 * mul)}
        ...
        self.l1_layers = nn.Sequential(
            nn.Conv2d(in_channels=my_blocks_input_shape["l1"], 
                      out_channels=my_blocks_output_shape['l1'], kernel_size=3, stride=2, padding=1, bias=False),
            nn.ReLU(),
            nn.BatchNorm2d(my_blocks_output_shape["l1"])
        )
        self.l2_layers = nn.Sequential(
            nn.Conv2d(in_channels=my_blocks_input_shape["l2"], 
                      out_channels=my_blocks_output_shape["l2"], kernel_size=3, stride=2, padding=1, groups=2, bias=False),
            nn.ReLU(),
            nn.BatchNorm2d(my_blocks_output_shape["l2"])
        )
        self.l3_layers = nn.Sequential(
            nn.Conv2d(in_channels=my_blocks_input_shape["l3"], 
                      out_channels=my_blocks_output_shape["l3"], kernel_size=3, stride=2, padding=1, groups=4, bias=False),
            nn.ReLU(),
            nn.BatchNorm2d(my_blocks_output_shape["l3"])
        )
        # self.l4_conv = nn.Conv2d(in_channels=resnet_blocks_output_shape['l4'], 
        #                          out_channels=my_blocks_output_shape["l4"], kernel_size=3, stride=2, padding=1, groups=8, bias=False)
        self.l4_layers = nn.Sequential(
            nn.Conv2d(in_channels=my_blocks_input_shape["l4"], 
                      out_channels=my_blocks_output_shape["l4"], kernel_size=3, stride=2, padding=1, groups=8, bias=False),
            nn.ReLU(),
            nn.BatchNorm2d(my_blocks_output_shape["l4"])
        )
